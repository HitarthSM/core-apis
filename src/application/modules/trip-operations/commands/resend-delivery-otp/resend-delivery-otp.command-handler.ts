...
import { ResendDeliveryOtpCommand, ResendDeliveryOtpResult } from './resend-delivery-otp.command';
...
@CommandHandlerStrict(ResendDeliveryOtpCommand)
export class ResendDeliveryOtpCommandHandler
  implements ICommandHandler<ResendDeliveryOtpCommand, ResendDeliveryOtpResult> {
  public constructor(
    @Inject(TRIP_STOP_REPO) private readonly stopRepo: ITripStopRepo,
    private readonly mailService: TripOperationsMailService,
    @InjectPinoLogger(ResendDeliveryOtpCommandHandler.name) private readonly logger: PinoLogger,
  ) {}

  public async execute(command: ResendDeliveryOtpCommand): Promise<ResendDeliveryOtpResult> {
    this.logger.info(`Executing Command '${ResendDeliveryOtpCommand.name}' stopId=${command.stopId}`);
    ...
    await this.mailService
      ...
      .catch((err: Error) =>
        this.logger.warn({ error: err.message }, 'OTP resend email failed — non-fatal'),
      );
