...
import { GetCostlyProductsQuery } from './get-costly-products.query';
...
const SQL = `
  ...
  HAVING AVG(bi.unit_price) > 0
  ORDER BY AVG(bi.unit_price) DESC
  LIMIT $5
`;

@QueryHandlerStrict(GetCostlyProductsQuery)
export class GetCostlyProductsHandler
  implements IQueryHandler<GetCostlyProductsQuery, ProductMarginRankResponse[]>
{
  public constructor(
    @InjectDataSource() private readonly dataSource: DataSource,
    @InjectPinoLogger(GetCostlyProductsHandler.name) private readonly logger: PinoLogger,
  ) {}

  public async execute(query: GetCostlyProductsQuery): Promise<ProductMarginRankResponse[]> {
    this.logger.info(`Executing Query '${GetCostlyProductsQuery.name}'`);
